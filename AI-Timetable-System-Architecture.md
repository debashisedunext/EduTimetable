...
1. **Setup Wizard** (multi-step, scoped to one timetable_config at a time): Academic Year → Classes/Sections → Rooms → Subjects → Curriculum mapping (class↔subject) → **Teachers** (list + add/edit — see 8.1a) → **Teacher Mapping** (Class-Teacher Assignments + Subject Mapping, both list + add/edit — see 8.1b) → Timetable Configuration (name, description, classes covered, days, periods, start time, breaks, zero period, computed end time)
