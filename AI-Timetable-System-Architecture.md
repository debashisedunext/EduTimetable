...
**Opening the door is not walking through it.** The run begins on the first *message*, server-side, where `chat_since` is stamped before anything is logged — not when the screen mounts. Starting it on mount meant a draft was created merely by looking, and a draft makes the welcome screen re-offer itself: somebody who opened the conversational setup and closed it again then met a modal on every page load afterwards. Relatedly, the welcome screen now auto-opens **once per browser tab**. `shouldPrompt` is a fact about the school and the draft and stays true until the setup is finished or discarded; read literally it puts a modal across the screen on every load, and a prompt that cannot be got past stops being an offer. The server still decides whether there is something to offer; the browser decides it has now been offered, per sitting rather than per person, with the Timetables screen's permanent entry point as the way to ask for it back.
